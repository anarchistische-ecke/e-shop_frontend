...
describe('product list url helpers', () => {
  it('parses and serializes the canonical catalog query schema', () => {
    const parsed = parseProductListSearchParams(
      '?query=Плед&scope=throws&brand=luna-soft&sort=priceAsc&minPrice=1000&maxPrice=5000&inStock=1&sale=1&page=3&original=плде',
      { source: 'catalog' }
    );
    ...
    expect(
      ...
      'query=%D0%9F%D0%BB%D0%B5%D0%B4&scope=throws&brand=luna-soft&sort=priceAsc&minPrice=1000&maxPrice=5000&inStock=1&sale=1&page=3&original=%D0%BF%D0%BB%D0%B4%D0%B5'
    );
  });
