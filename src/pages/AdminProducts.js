...
function AdminProducts() {
  ...
  useEffect(() => {
    ...
    getCategories().then((data) => {
                  ...
                  setCategories(cats);
                  // Set default category for newItem if not already selected
                  if (cats.length > 0) {
  ...
  const handleAddNew = async (e) => {
    ...
    try {
      ...
      // Create product (excluding price, category, brand at creation if not supported by API)
      const created = await createProduct({
        name: newItem.name,
        description: newItem.description,
        slug: slug
      });
      // Associate category and brand via update (if API supports it)
      if (newItem.category) {
        await updateProduct(created.id, { category: newItem.category, brand: newItem.brand });
      }
